...
public abstract class ProfileModelBase : PageModel
{
        ...
        IGameTrackingService gameTrackingService)
    {
        ...
        var friendIds = friendsOfProfileUserTask.Result.Select(f => f.Id).ToList();
        var activeFriends = new List<FriendDto>();
        foreach (var friendRelationship in friendsOfProfileUserTask.Result)
        {
            
            string friendUserId;
            if (friendRelationship.SenderId == userId)
            {
                friendUserId = friendRelationship.ReceiverId;
            }
            else if (friendRelationship.ReceiverId == userId)
            {
                friendUserId = friendRelationship.SenderId;
            }
            else
            {
            ...
            var userStatusInfo = await authService.SearchUserByIdAsync(friendUserId);


            if (userStatusInfo != null && userStatusInfo.status?.Equals("active", StringComparison.OrdinalIgnoreCase) == true)
            {
                activeFriends.Add(friendRelationship); 
            }
            
        }


        Console.WriteLine($"DEBUG: Final active friends count: {activeFriends.Count}");

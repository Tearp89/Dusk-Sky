...
public class PlaylistProfileModel : ProfileModelBase
{
    ...
    // pero si se usan en otros métodos de PlaylistProfileModel, deben ser campos privados aquí.
    ...
    private readonly ILogger<PlaylistProfileModel> _logger; // ✅ Declaración del logger
    ...
    public PlaylistProfileModel(
        ...
        ILogger<PlaylistProfileModel> logger) // ✅ Inyección de ILogger
    {
    ...
    public async Task<IActionResult> OnGetAsync(string userId)
    {
        ...
        try
        {
            ...
            backlogGameIdsStrings ??= new List<string>(); // ✅ Asegurar que no sea nulo
            ...
            var backlogGameIds = backlogGameIdsStrings
                .Where(id =>
                {
                    ...
                    _logger.LogWarning("OnGetAsync: ID de juego de backlog inválido '{InvalidId}' encontrado para el usuario '{ProfileUserId}'.", id, userId); // ✅ Registro de advertencia
                    return false;
                })
                .Select(Guid.Parse)
                .ToList();
            ...
            {
                if (gameId == Guid.Empty) // Este check es redundante después del Where(Guid.TryParse) pero es una buena defensa.
                {
